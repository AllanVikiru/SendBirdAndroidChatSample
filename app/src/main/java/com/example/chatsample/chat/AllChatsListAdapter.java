...
import com.example.chatsample.utils.DateUtils;
import com.example.chatsample.utils.FileUtils;
import com.example.chatsample.utils.TextUtils;
import com.example.chatsample.utils.TypingIndicator;
...
class AllChatsListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
...
interface OnItemLongClickListener {
    ...
    AllChatsListAdapter(Context context) {
        mContext = context;
    ...
    void updateOrInsert(BaseChannel channel) {
        ...
        for (int i = 0; i < mChannelList.size(); i++) {
            if (mChannelList.get(i).getUrl().equals(groupChannel.getUrl())) {
                ...
                notifyDataSetChanged();
                Log.v(AllChatsListAdapter.class.getSimpleName(), "Channel replaced.");
                return;
            }
        }
